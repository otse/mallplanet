...
export class pixel {
	constructor(
		public readonly parent: colormap,
	...
	connections() {
		let same = 0;
		if (this.left().is_color(this.data)) same++;
		if (this.right().is_color(this.data)) same++;
		if (this.top().is_color(this.data)) same++;
		if (this.bottom().is_color(this.data)) same++;
		return same;
	}
	is_color(vec: vec3 | vec4) {
